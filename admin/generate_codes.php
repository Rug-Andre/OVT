...
$voter_code_message = ''; // Message for voter code generation
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['title'])) {
    ...
    if ($result->num_rows == 0) {
        $election_message = "Error: Invalid user ID. User does not exist.";
...
function generateUniqueCode($conn) {
    ...
    do {
        ...
        $check = mysqli_query($conn, "SELECT voter_code FROM voter_codes WHERE voter_code = '$code'");
    } while (mysqli_num_rows($check) > 0);
    return $code;
}

// Handle voter code generation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['num_codes'])) {
    $election_id = intval($_POST['election_id']);
    $num_codes = intval($_POST['num_codes']);

    if ($election_id <= 0 || $num_codes <= 0) {
        $voter_code_message = "Invalid election ID or number of codes.";
    } else {
        for ($i = 0; $i < $num_codes; $i++) {
            $code = generateUniqueCode($conn);
            $code = mysqli_real_escape_string($conn, $code);
            $query = "INSERT INTO voter_codes (election_id, voter_code) VALUES ('$election_id', '$code')";
            if (!mysqli_query($conn, $query)) {
                $voter_code_message = "Error generating codes: " . mysqli_error($conn);
                break;
            }
        }
        if (empty($voter_code_message)) {
            $voter_code_message = "Successfully generated $num_codes voter codes!";
        }
    }
}
...
// Fetch elections for voter code generation
$elections_query = "SELECT election_id, election_name FROM elections WHERE created_by = '{$_SESSION['user_id']}'";
...
<head>
    ...
    <style>
        ...
        .popup .btn-primary:hover, .popup .btn-success:hover {
...
<body>
    ...
    <div class="main-content">
        <div class="dashboard">
            ...
            <div class="grid-container">
                ...
                <div class="grid-item" id="generate-codes-btn"><i class="fas fa-key"></i><a href="#" onclick="showVoterCodePopup(); return false;">Generate election codes</a></div>
    ...
    <!-- Voter Code Popup -->
    <div id="voter-code-overlay" class="overlay"></div>
    <div id="voter-code-popup" class="popup">
        <h2><i class="fas fa-key"></i> Generate Voter Codes</h2>
        <?php if (!empty($voter_code_message)): ?>
            <div class="alert-info"><i class="fas fa-info-circle"></i> <?php echo $voter_code_message; ?></div>
        <?php endif; ?>
        <form method="POST">
            <div class="form-group">
                <label class="form-label">Select Election</label>
            ...
            <div class="form-group">
                ...
                <input type="number" name="num_codes" class="form-control" min="1" required>
            </div>
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-plus"></i> Generate Codes
            </button>
            <button type="button" class="btn btn-secondary" onclick="hideVoterCodePopup()">
                <i class="fas fa-times"></i> Close
            </button>
        </form>
    </div>
    ...
    <script>
        ...
        // Voter Code Popup Functions
        const voterCodeOverlay = document.getElementById('voter-code-overlay');
        const voterCodePopup = document.getElementById('voter-code-popup');

        function showVoterCodePopup() {
            voterCodeOverlay.style.display = 'block';
            voterCodePopup.style.display = 'block';
            document.body.classList.add('overlay-active');
        }

        function hideVoterCodePopup() {
            voterCodeOverlay.style.display = 'none';
            voterCodePopup.style.display = 'none';
            document.body.classList.remove('overlay-active');
        }

        voterCodeOverlay.addEventListener('click', hideVoterCodePopup);
