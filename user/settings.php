...
$query = "SELECT username, email, role, created_at, image FROM users WHERE user_id = '$user_id'";
...
if (!$user) {
    // Log the issue and redirect
    error_log("No user found for user_id: $user_id");
    session_destroy(); // Clear session if user not found
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['update_image'])) {
    ...
    if (!empty($image)) {
        ...
        if (mysqli_query($conn, $update_query)) {
            ...
            $message = "Image is too large or update failed: " . mysqli_error($conn);
            $message_class = 'error';
        }
    } else {
    ...
    if (!$pass_result) {
        ...
        if (password_verify($current_password, $current_hashed_password)) {
            if ($new_password === $confirm_password) {
                ...
                $update_pass_query = "UPDATE users SET password = '$new_hashed_password' WHERE user_id = '$user_id'";
    ...
    <div class="main-content">
        <div class="settings-container">
            ...
            <form method="POST" action="settings.php">
                ...
                <div class="form-group">
                    <label for="role"><i class="fas fa-user-tag"></i> Role (Read-only)</label>
                    <input type="text" id="role" value="<?php echo htmlspecialchars($user['role']); ?>" readonly>
                </div>
                <div class="form-group">
                    <label for="created_at"><i class="fas fa-calendar-alt"></i> Account Created (Read-only)</label>
            ...
            <form method="POST" action="settings.php" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label><i class="fas fa-portrait"></i> Current Image</label>
                ...
                <div class="form-group">
                    <label for="update_image"><i class="fas fa-upload"></i> Upload New Image</label>
                    <input type="file" id="update_image" name="update_image" accept="image/*" required>
                </div>
                <div class="form-group">
                    <button type="submit" name="update_image"><i class="fas fa-save"></i> Update Image</button>
                </div>
            </form>
            ...
            <form method="POST" action="settings.php">
                ...
                <div class="form-group">
                    <button type="submit" name="change_password"><i class="fas fa-save"></i> Change Password</button>
                </div>
            </form>
